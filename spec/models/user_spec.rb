...
RSpec.describe User, type: :model do
  ...
  context "name" do
    it 'should be present' do
      ...
      expect(subject.errors.full_messages).to include("Name can't be blank", "Name is too short (minimum is 2 characters)")
    end
  
    it "should be between 2 and 50 characters" do
      valid_names = ["John", "Jane Doe", "A" * 50]
      valid_names.each do |name|
        new_user = User.new(name: name, email: "test@example.com", password: "password")
  ...
  context "email" do
    ...
    it "requires a valid email format" do
      ...
      invalid_emails.each do |email|
        new_user = User.create(name: 'new_user', email: email, password: 'new_password')
